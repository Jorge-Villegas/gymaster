...
class RutinaDataSource implements IRutinaDataSource {
  ...
  Future<Rutina> updateRutina(Rutina rutina) async {
    final db = await _databaseHelper.database;
    final res = await db.update(DatabaseHelper.tbRutina, rutina.toJson(),
        where: 'id = ?', whereArgs: [rutina.id]);
  ...
  Future<bool> deleteRutina(String id) async {
    final db = await _databaseHelper.database;
    final res = await db
        .delete(DatabaseHelper.tbRutina, where: 'id = ?', whereArgs: [id]);
  ...
  Future<List<Rutina>> getRutinasByEstado(int estado) async {
    final db = await _databaseHelper.database;
    final res = await db.query(DatabaseHelper.tbRutina,
        where: 'estado = ?', whereArgs: [estado]);
    return res.isNotEmpty ? res.map((c) => Rutina.fromJson(c)).toList() : [];
  }

  @override
  Future<List<Rutina>> getRutinasByFechaRealizacion(
      String fechaRealizacion) async {
    final db = await _databaseHelper.database;
    final res = await db.query(DatabaseHelper.tbRutina,
        where: 'fecha_realizacion = ?', whereArgs: [fechaRealizacion]);
    return res.isNotEmpty ? res.map((c) => Rutina.fromJson(c)).toList() : [];
  }
  ...
  Future<List<Rutina>> getRutinasByFechaCreacion(String fechaCreacion) async {
    final db = await _databaseHelper.database;
    final res = await db.query(DatabaseHelper.tbRutina,
        where: 'fecha_creacion = ?', whereArgs: [fechaCreacion]);
    return res.isNotEmpty ? res.map((c) => Rutina.fromJson(c)).toList() : [];
  }
  ...
  Future<List<Rutina>> getRutinasByColor(int color) async {
    final db = await _databaseHelper.database;
    final res = await db
        .query(DatabaseHelper.tbRutina, where: 'color = ?', whereArgs: [color]);
    return res.isNotEmpty ? res.map((c) => Rutina.fromJson(c)).toList() : [];
  }
  ...
  @override
  Future<List<Rutina>> getRutinasByEstadoAndFechaRealizacion(
      int estado, String fechaRealizacion) async {
    final db = await _databaseHelper.database;
    final res = await db.query(DatabaseHelper.tbRutina,
        where: 'estado = ? AND fecha_realizacion = ?',
        whereArgs: [estado, fechaRealizacion]);
    return res.isNotEmpty ? res.map((c) => Rutina.fromJson(c)).toList() : [];
  }

  @override
  Future<List<Rutina>> getRutinasByEstadoAndFechaCreacion(
      int estado, String fechaCreacion) async {
    final db = await _databaseHelper.database;
    final res = await db.query(DatabaseHelper.tbRutina,
        where: 'estado = ? AND fecha_creacion = ?',
        whereArgs: [estado, fechaCreacion]);
    return res.isNotEmpty ? res.map((c) => Rutina.fromJson(c)).toList() : [];
  }
}
